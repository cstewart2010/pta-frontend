<template>
    <div class="col-12 col-md-6 text-dark bg-success bg-gradient" style="text-align: center;">
        <div class="row">
        ...
        <div class="row" v-for="(item, index) in items" :key="index">
            ...
            <button class="col-1 btn btn-success bg-gradient border-bottom border-start" @click="setUseItem(item)"  data-bs-toggle="modal" data-bs-target="#useItemModal-berry">
                {{item.amount}}
            </button>
            <div class="col-8 border-bottom border-start">
                {{item.effects}}
        ...
        <div class="row border-bottom border-start">
            <select class="form-select m-1 col text-dark bg-success" name="item" style="max-width: 150px" v-model="addedItem">
                <option v-for="(item, index) in availableItems" :key="index" :id="item" :value="index + 1">
                    {{item}}
                </option>
            </select>
            <input type="number" class="m-1 form-control col text-dark bg-success bg-gradient"  min="1" name="item-amount" id="item-amount" v-model="addedAmount">
            <button class="btn btn-success bg-gradient m-1 col text-dark" @click="addItem">Add more items</button>
        </div>
    </div>
